...
class LuaGlobal extends LuaPresetBase
{
    public function new(lua:LuaScript)
    {
        ...
        {
            if (variables.exists(tag) && variables.get(tag) is FlxBasic)
                game.add(variables.get(tag));
            else
                game.debugPrint('Error: This Object Cannot be Added', FlxColor.RED);
        });
        ...
        set("getPropertyFromClass", function(classVar:String, variable:String, ?allowMaps:Bool = false)
            {
                ...
                if (myClass == null)
                {
                    game.debugPrint('getPropertyFromClass: Class $classVar not found', FlxColor.RED);
        ...
        set("getPropertyFromGroup", function(obj:String, index:Int, variable:Dynamic, ?allowMaps:Bool = false)
            {
                ...
                game.debugPrint("getPropertyFromGroup: Object #" + index + " from group: " + obj + " doesn't exist!", FlxColor.RED);
        ...
        set("createInstance", function(variableToSave:String, className:String, ?args:Array<Dynamic> = null)
            {
                ...
                if (!variables.exists(variableToSave))
                {
                    ...
                    if (myType == null)
                    {
                        game.debugPrint('createInstance: Variable $variableToSave is already being used and cannot be replaced!', FlxColor.RED);
                    ...
                    if (obj != null)
                        ...
                        game.debugPrint('createInstance: Failed to create $variableToSave, arguments are possibly wrong.', FlxColor.RED);
...
class LuaSprite extends LuaPresetBase
{
    public function new(lua:LuaScript)
    {
        ...
        set('createSprite', function(tag:String, x:Float, y:Float, sprite:String)
        {
            variables.set(tag, new FlxSprite(x, y, Paths.image(sprite)));
        });
    }
}
