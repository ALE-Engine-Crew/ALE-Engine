...
class CoolUtil
{
	...
	public static function loadSong(name:String, difficulty:String):Void
	{
		...
		for (folder in FileSystem.readDirectory('assets/data'))
		{
			...
			{
				if (Paths.fileExists('data/' + folder + '/' + name + difficulty + '.json'))
				{
					jsonData = tjson.TJSON.parse(sys.io.File.getContent(Paths.getPath('data/' + folder + '/' + name + difficulty + '.json')));
				} else {
					trace('Missing File: data/' + folder + '/' + name + difficulty + '.json');
				}
			}
		}
