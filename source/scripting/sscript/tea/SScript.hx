...
import sys.io.File;
#end
import scripting.sscript.tea.backend.*;
import scripting.sscript.tea.backend.crypto.Base32;
...
**/
@:structInit
@:access(scripting.sscript.hscriptBase.Interp)
@:access(scripting.sscript.hscriptBase.Parser)
@:keepSub
class SScript {
	/**
		Ignore return value 
	...
	**/
	public function new(?scriptPath:String = "", ?preset:Bool = true, ?startExecute:Bool = true) {
		...
		if (defines == null) {
			...
			var path:String = scripting.sscript.macro.Macro.definePath;
	...
	**/
	public function set(key:String, obj:Dynamic):SScript {
		...
		function setVar(key:String, obj:Dynamic):Void {
			...
			else if (obj != null && scripting.sscript.macro.Macro.macroClasses.contains(obj))
